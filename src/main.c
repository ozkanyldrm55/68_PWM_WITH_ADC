/*PA0 bacagina potansiyometre caglar ve degerini degistirirsek ledlerin parlakligi degisecek*/
...
void ADC_Config(void)
{
	...
	ADC_InitStruct.ADC_Resolution = ADC_Resolution_12b;//cozunurlugunu max ayarladýk ancak bu durum cok da iyi degil ses vs herseyden etkilenir
...
uint16_t Read_ADC()
{
	...
	ADC_SoftwareStartConv(ADC1);//ADC yi yazilimsal olarak baslattik //adc okumasý bittiginde EOC bayragi kalkar asagida while ile bu durum olusana kadar bekledik
	...
	return ADC_GetConversionValue(ADC1);//ADC degisim degerlerini okumak icin bu degerleri kullandýk.yani adc degerini cektik
